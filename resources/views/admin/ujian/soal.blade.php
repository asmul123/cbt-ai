...
<div class="card mb-3">
    <div class="card-header">
        <h6 class="mb-0"><i class="bi bi-list-check"></i> Pilih Soal untuk: {{ $ujian->nama_ujian }}</h6>
    </div>
    <div class="card-body">
        <div class="row mb-2">
            <div class="col"><strong>Mapel:</strong> {{ $ujian->mapel->nama ?? '-' }}</div>
            <div class="col"><strong>Status:</strong> <span class="badge bg-secondary">{{ ucfirst($ujian->status) }}</span></div>
            <div class="col"><strong>Soal terpilih:</strong> <span class="badge bg-primary" id="countSelected">{{ $ujian->soal->count() }}</span></div>
        </div>
    </div>
</div>

<form action="{{ route('admin.ujian.soalSync', $ujian) }}" method="POST">
    @csrf
    <div class="card">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="5" class="text-center text-muted py-4">Belum ada soal untuk mapel ini. Pastikan guru sudah membuat soal dengan status aktif.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div class="mt-3 d-flex gap-2">
        ...
        <a href="{{ route('admin.ujian.index') }}" class="btn btn-secondary">Kembali</a>
    </div>
</form>
@endsection
